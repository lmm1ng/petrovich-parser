...
class Parser {
    ...
    async fetch({url, method, params}) {
        ...
        return this.axios({ url, method, params: {'city_code': this.cityCode, ...params}, headers: `Cookie: ${cookiesString}`, withCredentials: true})
    }

    async testFetch() {
        const { data } = await this.fetch({url: '/catalog/v4/sections/1384/products', method: 'GET', params: {limit: this.limit}})
        console.dir(data)
    }
}
